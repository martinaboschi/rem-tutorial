...
a)  Define the covariate: $z_{s}(t)$: \textcolor{cerulean}{involvement of species} $s$ in first record occurring at $t$. Create a $n \times 2$ matrix \texttt{sp} containing in the fist column \texttt{spec} levels related to species involved in the events and in the second those involved in the non-events. Is it node- or dyad- level?
...
It is just reporting the label of the involved species $s$, so the covariate $z_{s}(t)$ is monadic. 
