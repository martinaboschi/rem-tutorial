...
- **Event log-rate contribution** $f\left( \boldsymbol{x}_{sr}(t),t \right)$. 
  E.g. **linear contribution**: $f\left( \boldsymbol{x}_{sr}(t),t \right) = \beta \cdot x_{sr}(t)$.
...
> $$\Delta N_{sr}(t_i)\mid \boldsymbol{x}_{sr}(t_i) \stackrel{\text{iid}}{\sim} \text{Poisson}\!\left(\mu_{sr}(t_i)\right), \qquad \log(\mu_{sr}(t_i)) = \boldsymbol{\beta} \cdot \boldsymbol{x}_{sr}(t_i) + \log(t_i - t_{i-1})$$
>
> **R code:**
> ```r
